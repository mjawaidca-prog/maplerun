...
export default async function ReportsDashboardPage() {
  ...
  const cards = [
    { href: "/reports/t4", icon: "📄", title: "T4 slips", desc: "Statement of remuneration paid for each employee. View, print, and file with the CRA.", metric: `${year} · available`, metricVal: "Ready", locked: false },
    { href: "/reports", icon: "🧾", title: "PD7A remittance", desc: "Source deductions owed to the CRA this period. Statement of account and remittance voucher.", metric: pd7aTotal > 0 ? `YTD ${year}` : "No data", metricVal: pd7aTotal > 0 ? fmtCAD(pd7aTotal) : "—", locked: false },
    { href: "/reports/roe", icon: "📋", title: "Record of Employment (ROE)", desc: "Generate ROEs for terminations and leaves, formatted for Service Canada submission.", metric: "", metricVal: "", locked: false },
    { href: "#", icon: "📊", title: "Payroll journal", desc: "Full debit/credit journal export for your accounting software, by pay run or date range.", metric: "Accountant plan", metricVal: "🔒", locked: true },
  ];
      ...
      <div className="grid grid-cols-2 gap-[18px]">
        {cards.map(card=>card.locked?(
          <div key={card.title} className="relative bg-[#FAFAF9] border border-[#E7E5E4] rounded-[14px] p-[22px] shadow-[0_1px_3px_rgba(0,0,0,0.04)]">
            ...
            <Link href="/upgrade" className="inline-flex items-center gap-1.5 mt-4 text-[13px] font-semibold bg-[#FFFBEB] border border-[#FDE68A] text-[#92400E] rounded-[9px] px-3.5 py-2 no-underline">Upgrade to unlock →</Link>
          </div>
        ):(
